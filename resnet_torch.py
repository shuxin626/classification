...
class ResNet(nn.Module):
    def __init__(self, in_ch, block, num_blocks, num_classes=4):
        super(ResNet, self).__init__()
        self.in_planes = 64

        self.conv1 = nn.Conv2d(in_ch, self.in_planes, kernel_size=3,
                               stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2) if num_blocks[2] is not None else None
...
if __name__ =="__main__":
    net = ResNet10(1, 4).to('cuda')
    print(summary(net, (1, 300, 300)))
